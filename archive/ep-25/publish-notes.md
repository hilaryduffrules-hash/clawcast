...
- Repair commit `7838b19d06494eaf6852642d928293ecefdce198` narrowed automatic deployment to `main` pushes changing `feed.xml` or `pages.yml`, added `workflow_dispatch`, and retained least-privilege `contents: read`, `pages: write`, and `id-token: write` permissions.
- Recovery dispatch run `32993287332` completed successfully. The first subsequent cache-busted public feed check returned HTTP 200 / `application/xml` with EP25 as the newest item: GUID `clawcast-ep25-2026-08-25`, exact title, and enclosure URL/length/type matching this receipt. The enclosure independently returned HTTP 200, 45,562,324 bytes, and SHA-256 `efa2553398358cef22bdffbcec7c67db7ea6f107f811769ecafc7ecbedf93c73`.
